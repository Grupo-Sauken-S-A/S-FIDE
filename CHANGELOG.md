...
### Agregado
- Hardware validado end-to-end: se confirma que `XMLSignerWindowsCSP`/`PDFSignerWindowsCSP` también funcionan correctamente contra los tres tokens ya certificados (SafeNet 5110+ L3, mToken CryptoID nueva, Feitian ePass2003) — actualizada la tabla de la sección 8 del manual técnico.
...
- `PDFSignerPKCS12`, `PDFSignerPKCS11` y `PDFSignerWindowsCSP`: un documento firmado con `-l true` (bloqueo = certificación DocMDP + cifrado) quedaba con la firma corrupta e ilegible — `PDFVerifySignatures` fallaba con "Unknown PdfException" ("Cannot decode PKCS#7 SignedData object") al intentar verificarlo, aunque el mismo documento firmado sin bloqueo verificaba bien. Causa: los tres firmadores primero firmaban el PDF sin cifrar y después volvían a serializar todo el documento a través de un `PdfWriter` cifrado, en una segunda pasada que no sabe que el campo `/Contents` de la firma debe quedar exento de cifrado. Se invirtió el orden — ahora se cifra primero el documento original (con una contraseña de propietario aleatoria de un solo uso, nunca persistida) y se firma después, en modo *append*, directamente sobre ese archivo ya cifrado, que es el patrón que espera iText para combinar cifrado y firma en un documento válido. Verificado con más de una decena de firmas `-l true`/`-l false` con y sin campo visible, re-verificación exitosa de todas, e intento de re-firmar un documento ya bloqueado (rechazado con mensaje controlado, sin stack trace, como corresponde).
